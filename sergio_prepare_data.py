...
def main():
    ...
    ap.add_argument(
        "--max_cells", type=int, default=22000,
        help="Cap on pooled cells actually WRITTEN for PseudoGRN to consume "
             ...
             "output), applied as min(natural_pool_size, max_cells) -- a "
             "tier already below the cap is left untouched, not force-set "
             "to the cap value. CORRECTED DEFAULT (was 3000 -- see "
             "PSEUDOGRN_RESULTS_ANALYSIS.md's Fix section): 3000 is smaller "
             "than even Tier 3's full 8,100-cell natural pool, so EVERY "
             "tier got capped to the identical 3000 cells, destroying the "
             "density-tier comparison this sweep exists to measure (same "
             "bug class independently found in this project's MTGRN "
             "harness). 22000 sits strictly ABOVE Tier 2/3's natural pools "
             "(13,500/8,100) -- so Tier 1 (40,500 natural) is the ONLY "
             "tier actually subsampled, down to 22,000; Tier 2 and Tier 3 "
             "pass through completely uncapped at their natural sizes, "
             "preserving tier ordering (22000 > 13500 > 8100). NOTE: a "
             "real timing benchmark on actual SERGIO data (not a small "
             "synthetic stand-in, unlike the original 3000 default) showed "
             "cal_mi2()+smooth() are considerably slower than originally "
             "estimated -- ~32 min total at just 3000 cells, projecting "
             "(linear-scaling assumption, unverified beyond one real data "
             "point) to roughly 2.4h at 13,500 cells and ~4h at 22,000 --"
             " accepted explicitly by the user rather than pursuing further "
             "timing investigation or a GPU path (see "
             "PSEUDOGRN_RESULTS_ANALYSIS.md's Fix section for the full "
             "reasoning and the investigation that was deliberately cut "
             "short here). Set to 0 to disable capping entirely (not "
             "recommended above ~5000 pooled cells given the above).",
    )
